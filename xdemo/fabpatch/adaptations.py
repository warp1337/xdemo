...
def _execute(task, host, my_env, args, kwargs, jobs, queue, multiprocessing, log):
    """
...
def execute_fab_patch(task, _queue, _job_queue, _exit_queue, log, *args, **kwargs):
    ...
    _queue if parallel else None
    # The original fabric version
    # jobs = JobQueue(pool_size, _queue)
    if state.output.debug:
        _job_queue._debug = True
        ...
        for host in my_env['all_hosts']:
            try:
                results[host] = _execute(
                    task, host, my_env, args, new_kwargs, _job_queue, _queue, multiprocessing, log
                )
